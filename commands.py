...
async def cmd_addstory(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Add a new story. Admin only."""
    is_allowed, error_msg, thread_id = await check_admin_and_forum(
        update, context, require_thread=False, allow_owner=True
    )
    if not is_allowed:
        await update.effective_message.reply_text(error_msg or "Доступ запрещен.")
        return
    ...
    if not context.args or len(context.args) < 2:
        await update.effective_message.reply_text(
            "Использование: /addstory <название> <подзаголовок> [image_url] [link_url] [order_index]\n\n"
            "Пример: /addstory \"Обмен валют\" \"Юань-Тенге\" https://example.com/image.jpg https://example.com 0"
        )
        return
    
    from database import db_add_story
    
    title = context.args[0]
    subtitle = context.args[1] if len(context.args) > 1 else None
    image_url = context.args[2] if len(context.args) > 2 else None
    link_url = context.args[3] if len(context.args) > 3 else None
    order_index = int(context.args[4]) if len(context.args) > 4 and context.args[4].isdigit() else 0
    
    story_id = db_add_story(title, subtitle, image_url, link_url, order_index)
    ...
    if story_id > 0:
        await update.effective_message.reply_text(
            ...
            f"ID: {story_id}\n"
            f"Название: {title}\n"
            f"Подзаголовок: {subtitle or 'Не указан'}\n"
            f"Изображение: {image_url or 'Не указано'}\n"
            f"Ссылка: {link_url or 'Не указана'}\n"
            f"Порядок: {order_index}"
        )
    else:
        await update.effective_message.reply_text("❌ Ошибка при добавлении сториса.")


async def cmd_delstory(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not context.args or len(context.args) < 1:
        await update.effective_message.reply_text(
            "Использование: /delstory <id>\n\n"
            "Используйте /liststories чтобы увидеть список сторисов и их ID."
        )
        return
    
    from database import db_delete_story
    
    try:
        story_id = int(context.args[0])
        if db_delete_story(story_id):
            await update.effective_message.reply_text(f"✅ Сторис #{story_id} удален.")
        else:
            await update.effective_message.reply_text(f"❌ Сторис #{story_id} не найден или ошибка при удалении.")
    except ValueError:
        await update.effective_message.reply_text("❌ ID должен быть числом.")


async def cmd_liststories(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """List all stories. Admin only."""
    is_allowed, error_msg, thread_id = await check_admin_and_forum(
        update, context, require_thread=False, allow_owner=True
    )
    if not is_allowed:
    ...
    try:
        ...
        cur = conn.execute("SELECT id, title, subtitle, image_url, link_url, order_index, active FROM stories ORDER BY order_index ASC, id ASC")
        all_stories = cur.fetchall()
        conn.close()
        
        if not all_stories:
            await update.effective_message.reply_text("📭 Сторисов пока нет. Используйте /addstory для добавления.")
            return
        
        lines = ["📚 Список сторисов:\n"]
        for story in all_stories:
            status = "✅ Активен" if story["active"] else "❌ Неактивен"
            lines.append(
                f"ID: {story['id']} | {status}\n"
                f"Название: {story['title']}\n"
                f"Подзаголовок: {story['subtitle'] or 'Не указан'}\n"
                f"Порядок: {story['order_index']}\n"
            )
...
async def cmd_togglestory(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not context.args or len(context.args) < 1:
        await update.effective_message.reply_text(
            "Использование: /togglestory <id>\n\n"
            "Переключает активность сториса (активен/неактивен)."
        )
        return
    
    from database import db_update_story
    import sqlite3
    from config import DB_PATH
    ...
    try:
        ...
        conn = sqlite3.connect(DB_PATH)
        conn.row_factory = sqlite3.Row
        cur = conn.execute("SELECT active FROM stories WHERE id=?", (story_id,))
        row = cur.fetchone()
        conn.close()
        
        if not row:
            await update.effective_message.reply_text(f"❌ Сторис #{story_id} не найден.")
            return
        
        new_active = 1 if row["active"] == 0 else 0
        if db_update_story(story_id, active=new_active):
            status = "активен" if new_active == 1 else "неактивен"
            await update.effective_message.reply_text(f"✅ Сторис #{story_id} теперь {status}.")
        else:
            await update.effective_message.reply_text(f"❌ Ошибка при обновлении сториса #{story_id}.")
    except ValueError:
        await update.effective_message.reply_text("❌ ID должен быть числом.")
    except Exception as e:
        logger.exception("Error toggling story")
        await update.effective_message.reply_text(f"❌ Ошибка: {str(e)}")
