...
- `/info [uuid]` — получить информацию о пользователе по UUID (только для админов, работает в теме форума). Если UUID не указан, используется UUID из БД для текущего пользователя темы.
